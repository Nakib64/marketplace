    ...
    <div className="bg-surface-container-low border border-outline-variant/30 rounded-xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-5 pb-2 border-b border-outline-variant/20">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-base font-bold text-on-surface">Proposal Terms &amp; Compensation</h2>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold text-on-surface flex items-center justify-between">
            ...
            <span className=" text-[11px] text-on-surface-variant">Suggested: 2-4 Weeks</span>
          </label>
          <div className="grid grid-cols-3 gap-2">
            {[2, 3, 4].map((w) => (
              <button
                ...
                className={`py-2 px-2 rounded-lg text-xs font-semibold transition-colors border ${durationWeeks === w
      ...
      <div className="flex flex-col gap-2">
        <span className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Payment Schedule</span>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div
            onClick={() => onStrategyChange('MILESTONE')}
            className={`p-3.5 rounded-xl cursor-pointer transition-all border ${strategy === 'MILESTONE' ? 'bg-surface-container border-primary shadow-sm' : 'bg-surface-container/50 border-outline-variant/30'
              }`}
          >
            <div className="flex items-center justify-between">
              ...
              <span className="px-1.5 py-0.5 rounded bg-surface-container-high text-primary  text-[10px] font-bold">RECOMMENDED</span>
            </div>
          </div>
          <div
            onClick={() => onStrategyChange('LUMP_SUM')}
            className={`p-3.5 rounded-xl cursor-pointer transition-all border ${strategy === 'LUMP_SUM' ? 'bg-surface-container border-primary shadow-sm' : 'bg-surface-container/40 border-outline-variant/30 opacity-70 hover:opacity-100'
              }`}
          >
            <div className="flex items-center gap-1.5">
              <span className="material-symbols-outlined text-on-surface-variant text-[18px]">payments</span>
              <span className="text-xs font-bold text-on-surface">Single Payment on Completion</span>
            </div>
          </div>
        </div>
      </div>
