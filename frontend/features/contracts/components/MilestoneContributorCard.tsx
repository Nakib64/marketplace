...
export const MilestoneContributorCard: React.FC<MilestoneContributorCardProps> = ({
  scopeTitle = 'Project Deliverable Scope',
  amount = 0,
  currency = 'BDT',
  contributorName = 'Freelancer',
  contributorEns,
  contributorRole = 'Verified Freelancer',
  successRate,
  submittedAgo,
  commitHash,
  onCopyHash,
  ...
  return (
    <div className="bg-surface-container rounded-2xl p-5 border border-outline-variant/30 flex flex-col gap-4 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-outline-variant/20">
        ...
        <div className="text-left sm:text-right">
          <span className="text-[10px] text-on-surface-variant uppercase tracking-wider font-semibold block">Protected Amount</span>
          <span className="text-xs font-bold text-on-surface">${amount.toLocaleString()} {currency} (Funded)</span>
        </div>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-4 p-3.5 rounded-xl bg-surface-container-low border border-outline-variant/20">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-surface-container-high flex items-center justify-center relative font-bold text-sm text-primary border border-outline-variant/30">
            {initial}
            ...
            {commitHash && (
              <div className="flex items-center gap-1 mt-0.5">
                ...
                <button
                  ...
                  className="text-primary hover:underline flex items-center gap-0.5 text-[11px] font-mono"
                >
